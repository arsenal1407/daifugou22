...
const RULE_LABELS = {
  ...
  sequenceRevolution: '階段革命(階段4枚以上で発動)',
  ...
  suitLock: '縛り(マーク)(同じマークが3回続くとそのマークしか出せない)',
  numberLock: '縛り(数字)(+1が3回続くと次も+1しか出せない)',
};
...
const EVENT_LABELS = {
  ...
  sevenGive: '7渡し!',
  tenDiscard: '10捨て!',
  suitLock: (e) => `${SUIT_SYMBOLS[e.suit] || ''}縛り!`,
  numberLock: '数字縛り!',
  revolution: (e) => (e.active ? '革命!!' : '革命終了'),
  elevenBack: (e) => (e.active ? '11バック!' : '11バック解除'),
};
...
socket.on('specialEvent', (data) => {
  ...
  eventQueue.push(...data.events);
  processEventQueue();
});

function processEventQueue() {
  if (eventShowing || eventQueue.length === 0) return;
  eventShowing = true;
  const ev = eventQueue.shift();
  const labelDef = EVENT_LABELS[ev.type];
  const text = typeof labelDef === 'function' ? labelDef(ev) : labelDef;
  const banner = el('specialBanner');
...
function cardEl(card, { selectable, selected, playable, onClick } = {}) {
  ...
  div.className =
    ...
    (playable ? ' playable' : '') +
    (selectable ? '' : ' static');
  div.innerHTML = card.joker
    ? `<div class="corner">JOKER</div><div class="suit">🃏</div>`
    : `<div class="corner">${info.label}<br>${info.symbol}</div><div class="suit">${info.symbol}</div><div class="corner corner-br">${info.label}<br>${info.symbol}</div>`;
...
function renderGame(state) {
  ...
  if (state.suitLockActive) flags.push(`${SUIT_SYMBOLS[state.suitLockSuit] || ''}縛り中`);
  if (state.numberLockActive) flags.push('数字縛り中');
  ...
  const playableIds = myTurn && state.playableCardIds ? new Set(state.playableCardIds) : null;
  sortHand(state.myHand).forEach((card) => {
    const selected = selectedCardIds.has(card.id);
    const playable = playableIds ? playableIds.has(card.id) : false;
